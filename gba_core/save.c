...
void GBA_DetectSaveType(u8 * romptr, int size)
{
    ...
    //DebugMessage("No save type string detected.");
    //GBA_ExecutionBreak();
...
{
    if(strlen(rom_path) > (MAXPATHLEN-1))
    {
...
u8 GBA_SaveRead8(u32 address)
{
    ...
    {
        //DebugMessage("Autodetect: Read byte from [%08X]", address);
    ...
    switch(SAVE_TYPE)
    {
        ...
        case SAV_EEPROM:
        {
            //DebugMessage("EEPROM: Read byte from [%08X]", address);
            return 1; //ready
        }
        case SAV_NONE:
        case SAV_AUTODETECT:
...
u16 GBA_SaveRead16(u32 address)
{
    if(SAVE_TYPE == SAV_AUTODETECT)
    {
        //DebugMessage("Autodetect: Read halfword from [%08X]", address);
    ...
    switch(SAVE_TYPE)
    {
        case SAV_EEPROM:
        {
            if(gba_dma3enabled())
            {
                //DebugMessage("EEPROM: DMA. Read halfword from [%08X]", address);
            ...
            {
                //DebugMessage("EEPROM: Read halfword from [%08X]", address);
                return 1; //ready
            }
        }
...
void GBA_SaveWrite8(u32 address, u8 data)
{
    if(SAVE_TYPE == SAV_AUTODETECT)
    {
        //DebugMessage("Autodetect: Write 8bit [%08X]=%02X",address,data);
    ...
    switch(SAVE_TYPE)
    {
        ...
        case SAV_FLASH512:
        {
            ...
            else if(FLASH_CMD_STATE == 2)
            {
                ...
                {
                    //if(data != 0xA0) DebugMessage("%02X",data);
        ...
        case SAV_EEPROM:
        {
            //DebugMessage("EEPROM: Write 8bit [%08X]=%02X",address,data);
            return;
        }
        case SAV_NONE:
        case SAV_AUTODETECT:
...
void GBA_SaveWrite16(u32 address, u16 data)
{
    if(SAVE_TYPE == SAV_AUTODETECT)
    {
        //DebugMessage("Autodetect: Write 16bit [%08X]=%04X",address,data);
    ...
    switch(SAVE_TYPE)
    {
        case SAV_EEPROM:
        {
            if(gba_dma3enabled())
            {
                ...
                //DebugMessage("EEPROM: Write 16bit [%08X]=%04X",address,data);
                ...
                else if(EEPROM_CMD_LEN == 2+EEPROM_ADDRESS_BUS)
                {
                    ...
                    {
                        //SETUP READING
                        //DebugMessage("EEPROM: READ %X",EEPROM_ADDRESS);
                        EEPROM_CMD_LEN = 0;
                ...
                else //WRITING
                {
                    ...
                    else if(EEPROM_DATA_STREAMING == 64)
                    {
                        ...
                        //DebugMessage("EEPROM: WRITE %X",EEPROM_ADDRESS);
...
void GBA_SaveWriteFile(void)
{
    switch(SAVE_TYPE)
    {
        case SAV_SRAM:
        {
            ...
            if(f == NULL)
            {
                ErrorMessage("Couldn't open file for saving.");
                return;
            }
            if(fwrite(SRAM_BUFFER, sizeof(SRAM_BUFFER), 1, f) != 1)
            {
                ErrorMessage("Couldn't save data to file.");
            }
            fclose(f);
            return;
        }
...
void GBA_SaveReadFile(void)
{
    switch(SAVE_TYPE)
    {
        case SAV_SRAM:
        {
            ...
            {
                ErrorMessage("Couldn't read data from file.");
            }
            fclose(f);
            return;
        }
        ...
        case SAV_EEPROM:
        {
            ...
            if((size != 512) && (size != 8*1024)) ErrorMessage("Corrupted saved data (wrong size).");
