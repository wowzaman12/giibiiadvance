        ...
        //if(Timer[0].cascade == 0)
        {
            ...
            while(Timer[0].remainingclocks <= 0)
            {
                ...
                if(REG_TM0CNT_L == 0)
                {
                    ...
                    REG_TM0CNT_L = Timer[0].start;
                    if(Timer[0].irqenable) GBA_CallInterrupt(BIT(3));
                    timer0overflowed = 1;
                }
            }

            returnclocks = Timer[0].remainingclocks * (0x10000-(u32)REG_TM0CNT_L);
        }
    }
    if(Timer[1].enabled)
    {
        if(Timer[1].cascade == 0)
        {
            ...
            while(Timer[1].remainingclocks <= 0)
            {
                ...
                if(REG_TM1CNT_L == 0)
                {
                    ...
                    REG_TM1CNT_L = Timer[1].start;
                    if(Timer[1].irqenable) GBA_CallInterrupt(BIT(4));
                    timer1overflowed = 1;
                }
            }

            returnclocks = min(returnclocks,Timer[1].remainingclocks * (0x10000-(u32)REG_TM1CNT_L));
        }
    ...
    if(Timer[2].enabled)
    {
        if(Timer[2].cascade == 0)
        {
            ...
            while(Timer[2].remainingclocks <= 0)
            {
                ...
                {
                    REG_TM2CNT_L = Timer[2].start;
                    if(Timer[2].irqenable) GBA_CallInterrupt(BIT(5));
                    timer2overflowed = 1;
                }
            }

            returnclocks = min(returnclocks,Timer[2].remainingclocks * (0x10000-(u32)REG_TM2CNT_L));
        }
    ...
    if(Timer[3].enabled)
    {
        if(Timer[3].cascade == 0)
        {
            ...
            while(Timer[3].remainingclocks <= 0)
            {
                ...
                {
                    REG_TM3CNT_L = Timer[3].start;
                    if(Timer[3].irqenable) GBA_CallInterrupt(BIT(6));
                }
            }

            returnclocks = min(returnclocks,Timer[3].remainingclocks * (0x10000-(u32)REG_TM3CNT_L));
        }
