...
void GBA_DMA0Setup(void)
{
    ...
    {
        DebugMessage("DMA 0 in mode 3 - prohibited");
        GBA_ExecutionBreak();
        DMA[0].enabled = 0;
    }
...
void GBA_DMA3Setup(void)
{
    ...
    if(REG_DMA3CNT_H & BIT(11)) { DebugMessage("Game Pak DRQ  - DMA3"); GBA_ExecutionBreak(); }
    //NOT EMULATED:
...
static inline s32 GBA_DMA0Update(s32 clocks) //return clocks to finish transfer
{
    ...
    {
        DebugMessage("DMA0, MODE: START_SPECIAL (?)");
        GBA_ExecutionBreak();
        DMA[0].enabled = 0;
        return 0x7FFFFFFF;
    }
    ...
    if(DMA[0].clocksremaining <= 0)
    {
        ...
        else
        {
            ...
            {
                //DebugMessage("DMA 0 immediate mode with repeat bit set.");
                //GBA_ExecutionBreak();
                REG_DMA0CNT_H &= ~BIT(15);
                DMA[0].enabled = 0;
                return 0x7FFFFFFF;
...
static inline s32 GBA_DMA1Update(s32 clocks) //return clocks to finish transfer
{
    ...
    if(DMA[1].clocksremaining <= 0)
    {
        ...
        else
        {
            ...
            {
                //DebugMessage("DMA 1 immediate mode with repeat bit set.");
                //GBA_ExecutionBreak();
                REG_DMA1CNT_H &= ~BIT(15);
                DMA[1].enabled = 0;
                return 0x7FFFFFFF;
...
static inline s32 GBA_DMA2Update(s32 clocks) //return clocks to finish transfer
{
    ...
    if(DMA[2].clocksremaining <= 0)
    {
        ...
        else
        {
            ...
            {
                //DebugMessage("DMA 2 immediate mode with repeat bit set.");
                //GBA_ExecutionBreak();
                REG_DMA2CNT_H &= ~BIT(15);
                DMA[2].enabled = 0;
                return 0x7FFFFFFF;
...
static inline s32 GBA_DMA3Update(s32 clocks) //return clocks to finish transfer
{
    ...
    {
        DebugMessage("DMA3, MODE: START_SPECIAL -- NOT EMULATED");
        GBA_ExecutionBreak();
        DMA[3].enabled = 0;
        return 0x7FFFFFFF;
    }
    ...
    if(DMA[3].clocksremaining <= 0)
    {
        ...
        else
        {
            ...
            {
                //DebugMessage("WARNING: DMA 3, immediate mode with repeat bit set.");
                //GBA_ExecutionBreak();
                REG_DMA3CNT_H &= ~BIT(15);
                DMA[3].enabled = 0;
                return 0x7FFFFFFF;
...
s32 GBA_DMAUpdate(s32 clocks)
{
    ...
    //if(dmaworking) return tempclocks;
    if(DMA[3].enabled) return GBA_DMA3Update(clocks);
...
void GBA_DMASoundRequestData(int A, int B)
{
    if(DMA[1].starttime == START_SPECIAL)
    {
        if( (A && (DMA[1].dstaddr==FIFO_A)) || (B && (DMA[1].dstaddr==FIFO_B)) )
        {
            ...
            int i; for(i = 0; i < 4; i++)
            {
            //    if(GBA_MemoryRead32(DMA[1].srcaddr)!=0) //DebugMessage("%08X",DMA[1].srcaddr);
            //    //DebugMessage("[%08x]= %08X",DMA[1].srcaddr,GBA_MemoryRead32(DMA[1].srcaddr));
            //    DebugMessage("%08X",GBA_MemoryRead32(DMA[1].srcaddr));
                GBA_MemoryWrite32(DMA[1].dstaddr,GBA_MemoryRead32(DMA[1].srcaddr));
