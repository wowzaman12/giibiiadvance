...
int Cartridge_Load(const u8 * pointer, const u32 rom_size)
{
	...
	switch(GameBoy.Emulator.HardwareType)
	{
		...
		default: //Should never happen
            ErrorMessage("Cartridge_Load(): Trying to load in an undefined mode, should never happen.");
            return 0;
			break;
	}
    ...
    if(EmulatorConfig.load_from_boot_rom)
    {
        ...
        if(boot_rom_filename != NULL)
        {
            if(GetRunningFolder())
            {
                char * completepath = malloc(strlen(GetRunningFolder())+strlen(boot_rom_filename)
                                            +strlen(BOOT_ROM_FOLDER)+2);
                sprintf(completepath,"%s/"BOOT_ROM_FOLDER"/%s",GetRunningFolder(),boot_rom_filename);
...
void SRAM_Save(void)
{
	...
	if(!savefile) ErrorMessage("Couldn't save SRAM.");
...
void RTC_Save(void)
{
	...
	if(!savefile) ErrorMessage("Couldn't save rtc data.");
