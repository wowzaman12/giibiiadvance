...
inline void GB_SerialClocks(int _clocks)
{
	...
	if(mem->IO_Ports[SC_REG-0xFF00] & 0x01) //Internal clock
	{
		...
		if(GameBoy.Emulator.serial_clocks > GameBoy.Emulator.serial_total_clocks)
		{
			...
			//if(mem->HighRAM[IE_REG-0xFF80] & I_SERIAL)
...
void GB_PrinterPrint(void)
{
	...
	for(i = 0; i < GBPRINTER_NUMPACKETS; i++)
	{
		...
		else
		{
			...
			while(1)
			{
				...
				if(size < 0) ErrorMessage("GB_PrinterPrint: size < 0 (%d)",size);
			}
		}
	}
	...
	while(1)
	{
		sprintf(filename,"%sprinter%d.png",completepath,printer_file_number);
    ...
    Save_PNG(filename,160,144,buf_temp);
...
bool GB_PrinterChecksumIsCorrect(void)
{
	...
	if(result != checksum) DebugMessage("GB Printer packet corrupted.\n"
...
void GB_PrinterExecute(void)
{
	switch(GB_Printer.cmd)
	{
		...
		case 0x04: //Data
			...
			{
				ErrorMessage("GB Printer packet limit reached.");
			}
			else
			{
...
//	DebugMessage("GB Printer - Received %02x",data);
	return;
}
