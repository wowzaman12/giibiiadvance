...
void Config_Load(void)
{
    ...
    if(tmp)
    {
        ...
        if(*tmp == '#')
        {
            ...
            int i;
            char aux[3]; aux[2] = '\0';
            for(i = 0; i < 2; i++) { aux[i] = *tmp; tmp ++; } u8 r = asciihextoint(aux);
            for(i = 0; i < 2; i++) { aux[i] = *tmp; tmp ++; } u8 g = asciihextoint(aux);
            for(i = 0; i < 2; i++) { aux[i] = *tmp; tmp ++; } u8 b = asciihextoint(aux);

            //DebugMessage("%02x %02x %02x",r,g,b);
